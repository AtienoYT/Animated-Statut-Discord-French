...
class AnimatedStatus {
	...
	getAuthor() {
		return "Modified By Atieno";
	}

	getDescription() {
		return "Anime ton Statut";
	}
	...
	Status_Animate(index = 0) {
		...
		if (this.animation[index] == undefined) {
			BdApi.showToast("Statut animé : Aucun statut défini. Allez dans Paramètres>Plugins pour définir une animation personnalisée !");
			return;
		}
	...
	getSettingsPanel() {
		...
		// timeout
		settings.appendChild(GUI.newLabel("Temp par Keyframe (En milliseconds)"));
